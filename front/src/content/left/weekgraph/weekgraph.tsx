...
function Weekgraph(props: any) {
  ...
  useEffect(() => {
    ...
    svg.append('path')
      ...
      .attr('stroke', '#3F8EF5')
      .attr('stroke-width', 2)
      .attr('transform', `translate(${margin.left},${margin.top})`);  // margin.top을 반영하여 이동

    // x축 추가
    svg.append('g')
      .attr('transform', `translate(${margin.left},${height + margin.top})`) // x축 위치 조정
      .call(d3.axisBottom(x));

    // y축 추가
    svg.append('g')
      .attr('transform', `translate(${margin.left},${margin.top})`) // y축 위치 조정
      .call(d3.axisLeft(y));
